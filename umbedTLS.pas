...
type
  ...
  mbedtls_ecdh_side                        = LongWord; // enum
  mbedtls_ecp_group_id                     = Longword; // enum
  ...
  uint16_t                                 = uint16;
  uint64_t                                 = uint64;
  mbedtls_mpi_uint                         = uint32;
  mbedtls_mpi_sint                         = int32;
  ...
  Pmbedtls_ecdh_context                    = ^mbedtls_ecdh_context;
  Pmbedtls_ecp_point                       = ^mbedtls_ecp_point;
  Pmbedtls_ecp_group                       = ^mbedtls_ecp_group;
  Pmbedtls_ecp_keypair                     = ^mbedtls_ecp_keypair;
  Pmbedtls_mpi                             = ^mbedtls_mpi;
  Pmbedtls_mpi_uint                        = ^mbedtls_mpi_uint;
  ...
  // unless otherwise noted, all these records / structures can be opaque
  ...
  mbedtls_mpi  = record // size 12
    s : integer;                  // integer sign
    n : size_t;                   // total # of limbs
    p : Pmbedtls_mpi_uint;        // pointer to limbs
  end;

  mbedtls_ecp_point = record // size 36
    X : mbedtls_mpi;              // the point's X coordinate
    Y : mbedtls_mpi;              // the point's Y coordinate
    Z : mbedtls_mpi;              // the point's Z coordinate
  end;

  mbedtls_ecp_group  = record // size 124 - this may need to be non opaque
    id : mbedtls_ecp_group_id;    // internal group identifier (4)
    P : mbedtls_mpi;              // prime modulus of the base field (12) 16
    A : mbedtls_mpi;              // 1. A in the equation, or 2. (A + 2) / 4 (12) 28
    B : mbedtls_mpi;              // 1. B in the equation, or 2. unused (12) 40
    G : mbedtls_ecp_point;        // generator of the (sub)group used (36) 76
    N : mbedtls_mpi;              // 1. the order of G, or 2. unused (12) 88
    pbits : size_t;               // number of bits in P (4) 92
    nbits : size_t;               // number of bits in 1. P, or 2. private keys (4) 96
    h : LongWord;                 // internal: 1 if the constants are static (4) 100
    modp : pointer;               // pointer function for fast reduction mod P (4) 104
    t_pre : pointer;              // pointer to function - unused (4) 108
    t_post : pointer;             // pointer to function - unused (4) 112
    t_data : Pointer;             // unused (4) 116
    T : Pmbedtls_ecp_point;       // pre-computed points for ecp_mul_comb() (4) 120
    T_size : size_t;              // number for pre-computed points (4) 124
  end;

  mbedtls_ecdh_context = record // size 308 - this needs to be non opaque
    grp : mbedtls_ecp_group;      // The elliptic curve used. (124)
    d : mbedtls_mpi;              // The private key. (12) 136
    Q : mbedtls_ecp_point;        // The public key. (36) 172
    Qp : mbedtls_ecp_point;       // The value of the public key of the peer. (36) 208
    z : mbedtls_mpi;              // The shared secret. (12) 220
    point_format : integer;       // The format of point export in TLS messages. (4) 224
    Vi : mbedtls_ecp_point;       // The blinding value. (36) 260
    Vf : mbedtls_ecp_point;       // The unblinding value. (36) 296
    _d : mbedtls_mpi;             // The previous \p d. (12) 308
   end;

  mbedtls_ecp_keypair  = record // size 172
    stuffing : array [0 .. 171] of byte;
  end;
 ...
 *)
  ...
  MBEDTLS_ECP_DP_SECP192R1                 = 1;  // 192-bits NIST curve
  MBEDTLS_ECP_DP_SECP224R1                 = 2;  // 224-bits NIST curve
  MBEDTLS_ECP_DP_SECP256R1                 = 3;  // 256-bits NIST curve
  MBEDTLS_ECP_DP_SECP384R1                 = 4;  // 384-bits NIST curve
  MBEDTLS_ECP_DP_SECP521R1                 = 5;  // 521-bits NIST curve
  MBEDTLS_ECP_DP_BP256R1                   = 6;  // 256-bits Brainpool curve
  MBEDTLS_ECP_DP_BP384R1                   = 7;  // 384-bits Brainpool curve
  MBEDTLS_ECP_DP_BP512R1                   = 8;  // 512-bits Brainpool curve
  MBEDTLS_ECP_DP_CURVE25519                = 9;  // Curve25519
  MBEDTLS_ECP_DP_SECP192K1                 = 10; // 192-bits "Koblitz" curve
  MBEDTLS_ECP_DP_SECP224K1                 = 11; // 224-bits "Koblitz" curve
  MBEDTLS_ECP_DP_SECP256K1                 = 12; // 256-bits "Koblitz" curve
...
procedure mbedtls_ecp_point_init (pt : Pmbedtls_ecp_point); cdecl; external;
procedure mbedtls_ecp_group_init (grp : Pmbedtls_ecp_group); cdecl; external;
procedure mbedtls_ecp_keypair_init (key : Pmbedtls_ecp_keypair); cdecl; external;
procedure mbedtls_ecp_point_free (pt : Pmbedtls_ecp_point); cdecl; external;
procedure mbedtls_ecp_group_free (grp : Pmbedtls_ecp_group); cdecl; external;
procedure mbedtls_ecp_keypair_free (key : Pmbedtls_ecp_keypair); cdecl; external;
function mbedtls_ecp_set_zero (pt : Pmbedtls_ecp_point) : integer; cdecl; external;
function mbedtls_ecp_is_zero (pt : Pmbedtls_ecp_point) : integer; cdecl; external;
function mbedtls_ecp_group_load (grp : Pmbedtls_ecp_group; id : mbedtls_ecp_group_id) : integer; cdecl; external;

procedure mbedtls_mpi_init (X : Pmbedtls_mpi); cdecl; external;
procedure mbedtls_mpi_free (X : Pmbedtls_mpi);cdecl; external;
function mbedtls_mpi_grow (X : Pmbedtls_mpi; nblimbs : size_t) : integer; cdecl; external;
function mbedtls_mpi_shrink (X : Pmbedtls_mpi; nblimbs : size_t) : integer; cdecl; external;
function mbedtls_mpi_copy (X : Pmbedtls_mpi; const Y : Pmbedtls_mpi) : integer; cdecl; external;
procedure mbedtls_mpi_swap (X : Pmbedtls_mpi; Y : Pmbedtls_mpi); cdecl; external;
function mbedtls_mpi_read_binary (X : Pmbedtls_mpi; const buf : PChar; buflen : size_t) : integer; cdecl; external;
function mbedtls_mpi_write_binary (X : Pmbedtls_mpi; buf : PChar; buflen : size_t) : integer; cdecl; external;

function mbedtls_mpi_lset (X : Pmbedtls_mpi; z : mbedtls_mpi_sint) : integer; cdecl; external;
function mbedtls_mpi_cmp_abs (const X : Pmbedtls_mpi; const Y : Pmbedtls_mpi) : integer; cdecl; external;
function mbedtls_mpi_cmp_mpi (const X : Pmbedtls_mpi; const Y : Pmbedtls_mpi) : integer; cdecl; external;

function mbedtls_ecdh_gen_public (grp : Pmbedtls_ecp_group; d : Pmbedtls_mpi; q : Pmbedtls_ecp_point;
                                  f_rng : TrngFunc; p_rng : Pointer ) : integer; cdecl; external;
function mbedtls_ecdh_compute_shared (grp : Pmbedtls_ecp_group; z : Pmbedtls_mpi;
                         const Q : Pmbedtls_ecp_point; const d : Pmbedtls_mpi;
                         f_rng : TrngFunc; p_rng : Pointer ) : integer; cdecl; external;
procedure mbedtls_ecdh_init (ctx : Pmbedtls_ecdh_context); cdecl; external;
procedure mbedtls_ecdh_free (ctx : Pmbedtls_ecdh_context); cdecl; external;
function mbedtls_ecdh_make_params (ctx : Pmbedtls_ecdh_context; olen : Psize_t;
                                   buf : PChar; blen : size_t;
                                   f_rng : TrngFunc; p_rng : Pointer) : integer; cdecl; external;
function mbedtls_ecdh_read_params (ctx : Pmbedtls_ecdh_context;
                                   const buf : PPChar; end_ : PChar) : integer; cdecl; external;
function mbedtls_ecdh_get_params (ctx : Pmbedtls_ecdh_context; const key : Pmbedtls_ecp_keypair;
                                  side : mbedtls_ecdh_side) : integer; cdecl; external;
function mbedtls_ecdh_make_public (ctx : mbedtls_ecdh_context; olen : PSize_t;
                                   buf : PChar; blen : size_t;
                                   f_rng : TrngFunc; p_rng : Pointer) : integer; cdecl; external;
function mbedtls_ecdh_read_public (ctx : Pmbedtls_ecdh_context;
                                   const buf : PChar; blen : size_t) : integer; cdecl; external;
function mbedtls_ecdh_calc_secret (ctx : Pmbedtls_ecdh_context; olen : Psize_t;
                                   buf : PChar; blen : size_t;
                                   f_rng : TrngFunc; p_rng : Pointer) : integer; cdecl; external;
